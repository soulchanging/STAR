...
class SucEncoder(nn.Module):
    def __init__(self, repr_dim, action_dim, hdim=1024, activ=F.elu):
        ...
        self.rr1 = nn.Linear(repr_dim , hdim)
        self.rr2 = nn.Linear(hdim, hdim)
        self.rr3 = nn.Linear(hdim, 1)
    ...
    def pred_reward1(self,obs,action):
        r = self.activ(self.r1(torch.cat([obs, action], 1)))
        r = self.activ(self.r2(r))
        r = self.r3(r)
        return r
    
    def pred_reward2(self,zsa):
        r = self.activ(self.rr1(zsa))
        r = self.activ(self.rr2(r))
        r = self.rr3(r)
        return r
...
class DrQV2Agent:
    def __init__(self, obs_shape, action_shape, device, lr, feature_dim,
                 ...
                 update_every_steps, stddev_schedule, stddev_clip, use_tb,alpha,beta):
        self.device = device
    ...
    def update_representation(self,obs,action,next_obs,reward,step):
        ...
        zsa_loss = F.mse_loss(pred_zs,next_obs) + self.beta*F.mse_loss(pred_zs.detach(),policy_zsa)
        
        
        r1 = self.sucencoder.pred_reward1(obs,action)
        r2 = self.sucencoder.pred_reward2(pred_zs)

        r_loss =  F.mse_loss(r1,reward) + F.mse_loss(r2,reward)
